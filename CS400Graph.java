...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    /**
     * Edge objects are stored within their source vertex, and group together their target
     * destination vertex, along with an integer weight.
     */
     ...
     * @throws IllegalArgumentException if either source or target or both are not in the graph, or
     *                                  if its weight is < 0
     ...
     * @throws IllegalArgumentException if either sourceVertex or targetVertex or both are not in
     *                                  the graph
     ...
     * weighted directed edges along this path. Path objects can be copied and extended to include
     * new edges and verticies using the extend constructor. In comparison to a predecessor table
     * which is sometimes used to implement Dijkstra's algorithm, this eliminates the need for
     * tracing paths backwards from the destination vertex to the starting vertex at the end of the
     * algorithm.
     */
    protected class Path implements Comparable<Path> {
        ...
        /**
         * Creates a new path containing a single vertex.  Since this vertex is both the start and
         * end of the path, it's initial distance is zero.
         *
         ...
         * affecting the original path object (copyPath). The path is then extended by the Edge
         * object extendBy.
         *
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            this.start = copyPath.start; // Set the start
            this.end = extendBy.target; // Set the end

            distance = copyPath.distance + extendBy.weight; // Extend the distance

            this.dataSequence = new LinkedList<>(); // Copy over the dataSequence
            this.dataSequence.addAll(copyPath.dataSequence);
            this.dataSequence.add(extendBy.target.data); // Add the extend by target
        }

        /**
         * Allows the natural ordering of paths to be increasing with path distance. When path
         * distance is equal, the string comparison of end vertex data is used to break ties.
         *
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        Hashtable<T, Vertex> visitedVertices = new Hashtable<>(); // Visited vertices
        PriorityQueue<Path> visitedPaths = new PriorityQueue<>(); // Visited Paths (with weights)
        Path path; // The path to be returned at the end

        // If start or end are null, or start/end isn't in the graph
        if(start == null || end == null || vertices.get(start) == null ||
            vertices.get(end) == null) {
            throw new NoSuchElementException("Start or end do not exist, or are not in the graph");
        }

        Vertex currVertex = vertices.get(start); // Starting vertex to visit
        visitedPaths.offer(new Path(currVertex)); // Pass to the pq, which is then looped through

        while (!visitedPaths.isEmpty()) {
            path = visitedPaths.poll(); // Grab the next path to look at
            currVertex = path.end; // Grab the current vertex to inspect

            if (visitedVertices.contains(currVertex)) { // If vertex has already been visited
                continue;
            }
            if (path.end.data.equals(end)) { // Shortest path has been found, return
                return path;
            }

            LinkedList<Edge> frontier = currVertex.edgesLeaving; // Edges to inspect for the vertex
            for (Edge edge : frontier) {
                Path copy = new Path(path, edge); // Extend the path by this edge
                visitedPaths.offer(copy);
            }
            visitedVertices.put(currVertex.data, currVertex);
        }
     ...
     * @return the cost of the shortest path between vertex with data item start and vertex with
     * data item end, including all edges between start and end
     * @throws NoSuchElementException when no path from start to end can be found including when no
     *                                vertex containing start or end can be found
     */
